...
Please refine and enhance the (orders) Resource in my Laravel Filament v4 eCommerce application.
...
Enhance Infolist presentation with a clear and attractive layout for (orders) details.
Make sure to add the required translation 
Ensure the UI aligns with Filament’s V4 best UX practices and provides a seamless workflow for managing (orders).
on the (orders) List(orders) add multiple tabs for filtration
Important: 
